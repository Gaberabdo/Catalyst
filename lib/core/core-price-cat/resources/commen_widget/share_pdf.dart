...
//   print(7777777);
// }
Future<void> generatePdf(context,{required Lists model}) async {
                  ...
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Link(
                            child: Text(
                                model.listOfItems?[index].iId?.image?.url??'https://www.nfet.net/nfet.jpg',
                            style: const TextStyle(
                              color: PdfColors.blue,
                              fontSize: 16,
                            )
                        ),
                            destination: model.listOfItems?[index].iId?.image?.url??'https://www.nfet.net/nfet.jpg'),
                        SizedBox(height: 20),
                        Column(
                            children: [
                              Text(
                                  'Name:  ${model.listOfItems?[index].iId?.name}',
                                  style: const TextStyle(
                                    color: PdfColors.amber,
                                    fontSize: 16,
                                  )
